...
export async function initializeSwap(swap: CnftSwapRequest): Promise<void | string> {
  ...
  if (isSwap == true) {
    ...
    const swapTreeAccount =
      await solanaAccountCompression.ConcurrentMerkleTreeAccount.fromAccountAddress(
        connection,
    ...
    swapProof = getSwapAssetProof.data.result.proof
      .slice(
        0,
        getSwapAssetProof.data.result.proof.length - (!!swapCanopyDepth ? swapCanopyDepth : 0),
      )
      .map((node: solanaWeb3.PublicKeyInitData) => ({
        pubkey: new solanaWeb3.PublicKey(node),
        isWritable: false,
  ...
  let lookupTableAccount: solanaWeb3.AddressLookupTableAccount | null = null;
  if (proof.length > 3) {
